...
class Network(object):
    ...
    def __init__(self, sizes):
        ...
        self.sizes = sizes
        self.parameters = {}
        for l in range(1, len(sizes)): # 1 , 2 , ... ,num of layers
    ...
    def cross_entropy_derivative(self, logits, y_true):
        ...
        """
        # Rec 6 , Slide 19 : the derivative of the loss respect to y_pred is = softmax(v_L) - y therfore:
        batch_size = y_true.shape[0]
        y_one_hot = np.eye(10)[y_true].T
        sm_output = softmax(logits, axis=0) 
        return (sm_output - y_one_hot)/batch_size
    ...
    def forward_propagation(self, X):
        ...
        """
        #Based on Rec6 , Slide20:
        forward_outputs = []
        z_prev = X.copy()
        v_l = None
        forward_outputs.append((0,X))
        for l in range(1, self.num_layers): #All layers exclude the last one
            W_l = self.parameters['W' + str(l)]
            b_l = self.parameters['b' + str(l)]
            v_l = np.dot(W_l, z_prev) + b_l

            #Activation:
            z_l = self.relu(v_l)
            forward_outputs.append((v_l , z_l))
            z_prev = z_l

        #last layer without activition:
        W_l = self.parameters['W' + str(self.num_layers)]
        b_l = self.parameters['b' + str(self.num_layers)]
        z_l = np.dot(W_l, z_prev) + b_l

        forward_outputs.append((z_l,z_l))

        return z_l, forward_outputs
    ...
    def backpropagation(self, ZL, Y, forward_outputs):
        ...
        for l in range(self.num_layers, 0, -1):
            v_l, z_l = forward_outputs[l]
            z_l_prev_layer = forward_outputs[l - 1][1]

            # Layer L
            if l == self.num_layers:
                dZ = self.cross_entropy_derivative(z_l, Y)
                dW = np.dot(dZ, z_l_prev_layer.T)
            # Layer L -1
            elif l == self.num_layers - 1:
                W_next = self.parameters['W' + str(l + 1)]
                dZ = np.dot(W_next.T, dZ)
                dW = np.dot((dZ * self.relu_derivative(v_l)), z_l_prev_layer.T)
            # Other 
            else:
                W_next = self.parameters['W' + str(l + 1)]
                v_l_next = forward_outputs[l + 1][0]
                dZ = np.dot(W_next.T, (self.relu_derivative(v_l_next) * dZ))
                dW = np.dot((dZ * self.relu_derivative(v_l)), z_l_prev_layer.T)
